...
const SKIP = new Set(['node_modules', '.git', 'dist', 'build', '.expo', 'android/app/build', 'ios/build']);
const ALLOWED = ['.ts', '.tsx', '.js', '.jsx', '.json', '.md', '.rules', '.xml', '.gradle', '.plist', '.env', '.yml', '.yaml'];
...
function walk(dir: string, root: string, out: FileRecord[] = []): FileRecord[] {
  for (const item of fs.readdirSync(dir)) {
    ...
    else if (stat.size < 350_000 && ALLOWED.some(ext => item.endsWith(ext) || item === ext)) {
...
export function scanProject(projectPath: string): ScanResult {
  ...
  const ctx: RuleContext = { files, hasFile: (name) => names.has(name) };
